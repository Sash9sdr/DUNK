...
export const MenuToggle: React.FC<MenuToggleProps> = ({ activeMenu, onToggle }) => {
  return (
    <div className="relative p-1 rounded-full flex w-full max-w-[340px] mx-auto bg-black/40 backdrop-blur-sm border border-white/10 shadow-inner">
      <motion.div
      ...
      <button
        ...
        className={`flex-1 relative z-10 py-2.5 text-[10px] sm:text-xs font-bold uppercase tracking-widest transition-colors duration-200 ${
          activeMenu === 'special' ? 'text-white text-shadow-sm' : 'hover:text-white'
        }`}
      >
        <span className={`relative ${activeMenu !== 'special' ? 'animate-special-blink' : 'text-white'}`}>
          Special
        </span>
      </button>
    </div>
